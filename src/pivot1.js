...
PivotGrid.prototype = {
    ...
    doLayout: function () {
        ...
        $('.pivot-left', this.el).css('padding-top', height);
    },
    ...
    createColumnHeader: function (sb, table) {
        ...
        for (var i = 0, l = table.length; i < l; i++) {
            ...
            for (var j = 0, k = row.length; j < k; j++) {
                ...
                sb[sb.length] = '<td class="pivot-headercell pivot-columncell" ';
                if (cell.colspan > 1) {
...
//    TreeUtil.each(group, function (node, index, parent, level) {
//        
//    }, 'items');
    ...
    //4) grandTotal
    var grandTotal = { value: 'Grand Total', field: '', name: '', level: 0, _group: true, total: true };
    grandTotal.data = [];
...
//    initFields(rowFields);
    ...
    var rowData = createGroupData(data, rowFields, dataFields, options);
    ...
    var TreeUtil = {
        ...
        each: function (nodes, fn, nodesField) {
            ...
            function each(nodes, parent, level) {
                ...
                for (var i = 0, l = nodes.length; i < l; i++) {
                    var node = nodes[i];
                    if (fn(node, i, parent, level) === false) {
                        isBreak = true;
                        return;
                    }
